...
class UXAuditor:
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'library', 'files', 'vendor'}]
            for file in files:
                if Path(file).suffix in extensions:
                    name_lower = file.lower()
                    if '.min.' in name_lower or any(p in name_lower for p in ['bootstrap', 'jquery', 'jscolor', 'fpdf', 'jscolor-usage']):
                        continue
